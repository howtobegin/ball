...
public class HttpTraceLogFilter extends OncePerRequestFilter implements Ordered {
    ...
    private Set<String> HEADERS = Sets.newHashSet("LANG", "BP-TOKEN", "B-TOKEN");
    /**
     * 如果uri是如下列表中的一项开头的，那么request不打印
     * 比如文件上传相关的，那就request不打印，但是打印response
     */
    private List<String> ignorePrintRequestUriList = Lists.newArrayList( "/proxy/user/file","/proxy/file");
    /**
     * 如果uri是如下列表中的一项开头的，那么response不打印
     * 比如文章相关的，就是打印request不打印response，比如针对一些返回私钥的接口也需要配置在这里，不打印私钥信息
     */
    private List<String> ignorePrintResponseUriList = Lists.newArrayList("/proxy/article/", "/proxy/file/download");

    /**
     * 需要脱敏的header头的key，在打印头的时候进行脱敏
     * 不区分大小写
     */
    private List<String> desenHeaderList = Lists.newArrayList("BP-TOKEN", "TOKEN", "PKEY", "KEY");
